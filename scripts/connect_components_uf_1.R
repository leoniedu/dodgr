...
if ((!file.exists(complete_net_0_fname))|(!use_cache)) {
  if ((!file.exists(complete_sf_fname))|(!use_cache)) {
    ...
    complete_map_sf <- readr::read_csv(complete_sf_fname)
  }
  complete_map_sf_r <- complete_map_sf%>%
    filter(!highway%in%waterway)
  complete_map_sf_w <- complete_map_sf%>%
    filter(highway%in%waterway)
  complete_net_r <- dodgr::weight_streetnet(complete_map_sf_r, id_col = "id", wt_profile_file = "~/github/dodgrconnect/data-raw/profile.js", wt_profile = "motorcar")
  complete_net_r <- complete_net_r%>%
    #mutate(graph_index=1:n())%>%
    mutate(edge_id=as.character(edge_id))
  complete_net_w <- dodgr::weight_streetnet(complete_map_sf_w, id_col = "id", wt_profile_file = "~/github/dodgrconnect/data-raw/profile.js", wt_profile = "motorcar")
  dim(complete_net_w)
...
pts_all <- pts[0,]
cmp_all <- unique(pts$component)%>%sort()
...
for (j in cmps_to_join[-1]) {
  ...
  complete_net_all <- add_nodes_to_graph_by_edge(
    ...
    wt_profile = "motorcar", highway = "artificial", wt_profile_file = "~/github/dodgrconnect/data-raw/profile.json",
...
if(nrow(complete_connected_w)>0) {
  complete_connected_w <- complete_connected_w%>%
    add_nodes_to_graph_by_edge(xy=tract_agencias, wt_profile = "motorcar", highway = "artificial", wt_profile_file = "~/github/dodgrconnect/data-raw/profile.json", xy_id = tract_agencias$id )
}
