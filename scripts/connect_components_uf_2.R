...
complete_connected_now <- complete_connected%>%
  filter(component==1)

if (max(pts_connected$component)>1) {
  stop("Handle cases where points are not connected to component 1")
}
pts_connected_now <- pts_connected%>%
  filter(component==1)
...
tmp <- dodgr_paths(complete_connected_now, from=
                     pts_connected[pts_connected$index=="1704412",c("x", "y")]
                   , to=
                     pts_connected[pts_connected$index=="1704436",c("x", "y")]
                   , vertices = FALSE)
 
 
#tmpg <- complete_connected[complete_connected$edge_id%in%unlist(tmp),]
tmpg <- complete_connected_now[unlist(tmp),]
...
dist_setor_agencia_cat <- dodgr_dists_categorical(
  complete_connected%>%transmute(edge_id, from_id, from_lat, from_lon, to_id, to_lat, to_lon, d=d, time=time_weighted, highway, edge_type=case_when(highway%in%waterways ~ "waterway",
...
ggplot(complete_map_sf, aes(color=highway%in%waterways))+
  ...
  geom_segment(aes(x = from_lon, y=from_lat, xend = to_lon, yend = to_lat),
               data=complete_connected[unlist(paths_setor_agencia[[1]][[3]]),], color="purple")+
...
(complete_connected[unlist(paths_setor_agencia[[1]][[3]]),])%>%
  group_by(highway)%>%
...
complete_connected_0 <- complete_net%>%
  ...
  add_nodes_to_graph_by_edge(xy=tracts%>%select(id=setor), xy_id = tracts$setor, 
                             highway = "artificial", wt_profile = "motorcar", wt_profile_file = "~/github/dodgrconnect/data-raw/profile.json", replace_component=FALSE)
