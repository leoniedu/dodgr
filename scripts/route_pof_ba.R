...
for (j in cmps_to_join[-1]) {
  ...
  rr <- vertices_j%>%
    select(from_id=id)%>%
    left_join(complete_net_j%>%select(from_id, highway))
  ww <- any(rr$highway%in%waterways)
  hw <- if(ww) "artificial_river" else "artificial_road"
  stopifnot(0==(rr%>%filter(is.na(highway))%>%nrow()))
  complete_net_all <- add_nodes_to_graph_by_edge(
    graph=complete_net_all,
    xy=vertices_j[,c("x","y")],
    intersections_only=FALSE,
    wt_profile = "motorcar", highway = hw, wt_profile_file = "data-raw/profile.json",
